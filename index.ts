...
import { apiLimiter, authLimiter, uploadLimiter } from './src/middleware/rateLimiter'
import { startScheduler } from './src/services/scheduler'
import { setIo, registerUserSocket, unregisterUserSocket } from './src/lib/socketBus'
...
io.on('connection', async (socket) => {
  ...
  try {
    ...
    if (!device) {
      ...
      logger.info(`New device registered: ${device.deviceName}`, { ip: clientIp, deviceId: device.id })
    } else {
      ...
      logger.info(`Device reconnected: ${device.deviceName}`, { ip: clientIp, deviceId: device.id })
    }
    ...
    logger.error('Error registering device', { error, socketId: socket.id, clientIp })
  }

  // Handle disconnect
  socket.on('disconnect', async () => {
    // Cleanup user room mapping
    try { unregisterUserSocket(socket) } catch {}
  ...
  socket.on('device:info', async (data) => {
    try {
      if (socket.data.deviceDbId) {
        ...
        logger.info(`Device info updated`, { deviceId: socket.data.deviceDbId, androidVersion: data.androidVersion })
      }
    } catch (error) {
      logger.error('Error updating device info', { error, socketId: socket.id })
    }
  })
...
if (isDev) {
  ...
  if (fs.existsSync(clientDist)) {
    ...
    logger.warn('Client dist folder not found. Run `bun run build:client` first.')
  }
}
