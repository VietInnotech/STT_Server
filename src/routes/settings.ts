 ...
 */
router.get('/system', requireRole('admin'), async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const systemSettings = await prisma.systemConfig.findMany({
      where: {
        key: {
 ...
 */
router.put('/system', requireRole('admin'), async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const updates = req.body
    ...
    await prisma.auditLog.create({
      data: {
        ...
        resource: 'settings',
        details: { updatedKeys: Object.keys(updates), updatedBy: req.user!.username },
        ipAddress: req.ip || req.socket.remoteAddress || 'unknown',
        userAgent: req.headers['user-agent'] || 'unknown',
      },
    })
 ...
 */
router.delete('/:key', async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({ error: 'Unauthorized' })
      return
...
router.get('/preferences', async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    res.json({
      ...
      defaultTemplateId: userSettings.defaultTemplateId || null,
    })
  } catch (error) {
...
router.put('/preferences', async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const { defaultDeleteAfterDays } = req.body as { defaultDeleteAfterDays?: number | null }

    // Validate input
    if (defaultDeleteAfterDays !== null && defaultDeleteAfterDays !== undefined) {
      if (typeof defaultDeleteAfterDays !== 'number' || defaultDeleteAfterDays < 1 || defaultDeleteAfterDays > 365) {
        res.status(400).json({ error: 'defaultDeleteAfterDays must be between 1 and 365, or null' })
        return
      }
    }
    ...
    await prisma.userSettings.upsert({
      where: { userId: req.user.userId },
      update: {
        defaultDeleteAfterDays: defaultDeleteAfterDays === null ? null : Number(defaultDeleteAfterDays),
      },
      create: {
        userId: req.user.userId,
        defaultDeleteAfterDays: defaultDeleteAfterDays === null ? null : Number(defaultDeleteAfterDays),
