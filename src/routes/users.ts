 ...
 *     description: Retrieve a list of all users with their roles (admin only)
 *     tags: [Users]
 *     security:
 *       - bearerAuth: []
 *     responses:
 ...
 * Admin only
 */
router.get('/:id', requireRole('admin'), async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params
  ...
  try {
    ...
    await prisma.auditLog.create({
      data: {
        ...
        details: { username: newUser.username, email: newUser.email, createdBy: req.user!.username },
...
router.put('/:id', requireRole('admin'), async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params
    const { username, email, fullName, roleId, isActive, password } = req.body
    ...
    await prisma.auditLog.create({
      data: {
        ...
        details: { username: updatedUser.username, updatedBy: req.user!.username, changes: Object.keys(updateData) },
        ipAddress: req.ip || req.socket.remoteAddress || 'unknown',
        userAgent: req.headers['user-agent'] || 'unknown',
      },
    })
 ...
 */
router.get('/roles/list', async (req: AuthRequest, res: Response): Promise<void> => {
  try {
