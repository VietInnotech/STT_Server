 ...
 *               $ref: '#/components/schemas/Error'
 */
router.post('/audio', uploadLimiter, upload.single('file'), requireRole('admin', 'user'), async (req: AuthRequest, res: Response): Promise<any> => {
  try {
    if (!req.file) {
      return res.status(400).json({ error: 'No file uploaded' })
    }

    const { deviceId, deleteAfterDays } = req.body as { deviceId?: string; deleteAfterDays?: number }
    ...
    if (finalDeleteAfterDays === undefined) {
      ...
      if (userSettings && userSettings.defaultDeleteAfterDays !== undefined && userSettings.defaultDeleteAfterDays !== null) {
    ...
    const allowedAudioExts = ['.wav', '.mp3', '.ogg', '.webm', '.m4a', '.aac', '.flac']
    const mime = req.file.mimetype || ''
    const ext = path.extname(req.file.originalname || '').toLowerCase()

    if (!allowedAudioMimes.includes(mime) && !allowedAudioExts.includes(ext)) {
      return res.status(400).json({ error: 'Invalid audio file type. Allowed: wav, mp3, ogg, webm, m4a, aac, flac' })
    }
    ...
    try {
      ...
      if (err?.code === 'P2003') {
        logger.error('Foreign key constraint when creating audio file', { err, deviceId, uploadedById })
        return res.status(400).json({ error: 'Invalid deviceId or uploadedById. Make sure the device and user exist.' })
      }
      throw err
    }
    ...
    try {
      await prisma.auditLog.create({
        data: {
          ...
          details: { filename: audioFile.filename, fileSize: audioFile.fileSize, mimeType: audioFile.mimeType, deviceId: resolvedDeviceId },
 ...
 *               $ref: '#/components/schemas/Error'
 */
router.post('/text', uploadLimiter, upload.single('file'), requireRole('admin', 'user'), async (req: AuthRequest, res: Response): Promise<any> => {
  try {
    if (!req.file) {
      return res.status(400).json({ error: 'No file uploaded' })
    }

  const { deviceId, deleteAfterDays, source } = req.body as { deviceId?: string; deleteAfterDays?: number; source?: string }
    const uploadedById = req.user?.userId // Get from authenticated user
    ...
    if (source === 'android') {
      ...
      const rawJson = (req.body && (req.body.json || req.body.data || req.body.payload)) as string | undefined
      ...
      if (rawJson) {
        try {
          ...
          return res.status(400).json({ error: 'Invalid JSON payload from android upload' })
        }
      } else if (req.file.mimetype === 'application/json' || req.file.mimetype === 'text/json') {
        try {
          ...
          return res.status(400).json({ error: 'Invalid JSON file from android upload' })
        }
      }
    }
    ...
    try {
      ...
      if (err?.code === 'P2003') {
        logger.error('Foreign key constraint when creating text file', { err, deviceId, uploadedById })
        return res.status(400).json({ error: 'Invalid deviceId or uploadedById. Make sure the device and user exist.' })
      }
      throw err
    }
    ...
    try {
      await prisma.auditLog.create({
        data: {
          ...
          details: { filename: textFile.filename, fileSize: textFile.fileSize, mimeType: textFile.mimeType, deviceId: resolvedDeviceId },
          ipAddress: req.ip || req.socket.remoteAddress || 'unknown',
          userAgent: req.headers['user-agent'] || 'unknown',
          success: true,
        },
 ...
 *               $ref: '#/components/schemas/Error'
 */
router.get('/audio', async (req: AuthRequest, res: Response): Promise<any> => {
  try {
    ...
    if (!isAdmin && req.user?.userId) where.uploadedById = req.user.userId
 ...
 */
router.get('/all', async (req: AuthRequest, res: Response): Promise<any> => {
  try {
    const { deviceId, limit = '50', offset = '0' } = req.query as any
    const isAdmin = req.user?.roleName === 'admin'
    ...
    if (!isAdmin && req.user?.userId) {
      const shares = await prisma.fileShare.findMany({
        ...
        select: { fileId: true, fileType: true, expiresInDays: true, expiresAt: true, sharedById: true },
      })
    ...
    const shareMap = new Map<string, { expiresInDays?: number | null; expiresAt?: Date | null; sharedById?: string | null; sharedByName?: string | null }>()
    if (!isAdmin && req.user?.userId) {
      ...
      const sharedByIds = Array.from(new Set(activeShares.map((s) => s.sharedById).filter(Boolean))) as string[]
      let sharedByUsers: any[] = []
      if (sharedByIds.length > 0) {
        sharedByUsers = await prisma.user.findMany({ where: { id: { in: sharedByIds } }, select: { id: true, username: true, fullName: true } })
      }
      const userNameMap: Record<string, string> = {}
      for (const u of sharedByUsers) userNameMap[u.id] = u.fullName || u.username

      for (const s of activeShares) {
        const sharedByName = s.sharedById ? (userNameMap[s.sharedById] || null) : null
        shareMap.set(s.fileId, { expiresInDays: s.expiresInDays ?? null, expiresAt: s.expiresAt ?? null, sharedById: s.sharedById ?? null, sharedByName })
      }
    }

    // For owners, attach list/count of shares they created for their files (to show shared indicator and allow revoke)
    const ownerShareMap = new Map<string, Array<{ sharedWithId: string; expiresAt?: Date | null }>>()
    if (req.user?.userId) {
      // collect file IDs returned to query owner shares only for those files
      const returnedFileIds = [...audioFiles.map(f => f.id), ...textFiles.map(f => f.id)]
      if (returnedFileIds.length > 0) {
        ...
        for (const s of ownerShares) {
          ...
          arr.push({ sharedWithId: s.sharedWithId, expiresAt: s.expiresAt ?? null })
    ...
    const audioEnriched = audioFiles.map((f) => ({ ...f, _share: shareMap.get(f.id) || null, _ownerShares: ownerShareMap.get(f.id) || null }))
    const textEnriched = textFiles.map((f) => ({ ...f, _share: shareMap.get(f.id) || null, _ownerShares: ownerShareMap.get(f.id) || null }))
 ...
 */
router.get('/text', async (req: AuthRequest, res: Response): Promise<any> => {
  try {
    const { deviceId, limit = '50', offset = '0' } = req.query as any
    const isAdmin = req.user?.roleName === 'admin'
 ...
 */
router.get('/audio/:id', async (req: AuthRequest, res: Response): Promise<any> => {
  try {
    const { id } = req.params
    ...
    if (!isAdmin && audioFile.uploadedById !== req.user?.userId) {
      ...
      logger.debug('Download permitted via active share', { fileId: id, sharedWith: req.user?.userId })
    }

    // Decrypt file content
    const decryptedData = decrypt(Buffer.from(audioFile.encryptedData), audioFile.encryptedIV)
 ...
 * Decrypts and returns file
 */
router.get('/text/:id', async (req: AuthRequest, res: Response): Promise<any> => {
  try {
    const { id } = req.params
    ...
    // Decrypt file content
    const decryptedData = decrypt(Buffer.from(textFile.encryptedData), textFile.encryptedIV)
 ...
 */
router.delete('/audio/:id', requireRole('admin', 'user'), async (req: AuthRequest, res: Response): Promise<any> => {
  try {
    const { id } = req.params
    const isAdmin = req.user?.roleName === 'admin'
    ...
    if (!isAdmin) {
      ...
      if (file.uploadedById !== req.user?.userId) {
        return res.status(403).json({ error: 'Forbidden: You cannot delete this file' })
      }
    }
 ...
 */
router.delete('/text/:id', requireRole('admin', 'user'), async (req: AuthRequest, res: Response): Promise<any> => {
  try {
    const { id } = req.params
    const isAdmin = req.user?.roleName === 'admin'
    ...
    if (!isAdmin) {
      ...
      if (!file) return res.status(404).json({ error: 'Text file not found' })
      if (file.uploadedById !== req.user?.userId) {
        return res.status(403).json({ error: 'Forbidden: You cannot delete this file' })
      }
    }
 ...
 */
router.post('/share', requireRole('admin', 'user'), async (req: AuthRequest, res: Response): Promise<any> => {
  try {
    const { shares } = req.body as { shares?: Array<{ fileId: string; fileType: string; userId: string }> }
    ...
    for (const s of shares) {
      ...
      if (fileType === 'audio') {
        const f = await prisma.audioFile.findUnique({ where: { id: fileId } })
        if (!f) continue
      ...
      try {
        ...
        logger.debug('Socket emit failed for share', { err: (emitErr as Error).message })
      }
...
router.post(
  ...
  async (req: AuthRequest, res: Response): Promise<any> => {
    try {
      ...
      if (!summary || !realtime) {
        return res.status(400).json({ error: 'Both summary and realtime content are required' })
      }
      ...
      const result = await prisma.$transaction(async (tx) => {
        ...
        const { encryptedData: summaryEncrypted, encryptedIV: summaryIV } = encrypt(Buffer.from(summary))
        const { encryptedData: realtimeEncrypted, encryptedIV: realtimeIV } = encrypt(Buffer.from(realtime))
  ...
  async (req: AuthRequest, res: Response): Promise<any> => {
    try {
      ...
      if (!summaryFile && !realtimeFile) {
        return res.status(400).json({ error: 'At least one file (summary or realtime) is required' })
      }

      const { deleteAfterDays, pairName } = req.body as { deleteAfterDays?: number; pairName?: string }
      const uploadedById = req.user?.userId
      if (!uploadedById) {
        return res.status(401).json({ error: 'User not authenticated' })
      }
      ...
      const result = await prisma.$transaction(async (tx) => {
        ...
        if (!summaryTextFile || !realtimeTextFile) {
          throw new Error('Both summary and realtime files must be provided to create a pair')
        }
 ...
 */
router.get('/pairs/:pairId', requireRole('admin', 'user'), async (req: AuthRequest, res: Response): Promise<any> => {
  try {
    ...
    if (!isAdmin && pair.uploadedById !== req.user?.userId) {
      return res.status(403).json({ error: 'Forbidden: You do not have access to this pair' })
    }
 ...
 */
router.get('/pairs', requireRole('admin', 'user'), async (req: AuthRequest, res: Response): Promise<any> => {
  try {
 ...
 *         description: Pair not found
 */
router.delete('/pairs/:pairId', requireRole('admin', 'user'), async (req: AuthRequest, res: Response): Promise<any> => {
  try {
    const { pairId } = req.params

    const pair = await prisma.textFilePair.findUnique({ where: { id: pairId } })
    if (!pair) {
      return res.status(404).json({ error: 'Pair not found' })
    }
    ...
    const isAdmin = req.user?.roleName === 'admin'
    if (!isAdmin && pair.uploadedById !== req.user?.userId) {
      return res.status(403).json({ error: 'Forbidden: You cannot delete this pair' })
    }
    ...
    return res.json({ success: true, message: 'Pair and both files deleted' })
  } catch (error) {
 ...
 */
router.delete('/share', requireRole('admin', 'user'), async (req: AuthRequest, res: Response): Promise<any> => {
  try {
    const { fileId, sharedWithId } = req.body as { fileId?: string; sharedWithId?: string }
    if (!fileId || !sharedWithId) return res.status(400).json({ error: 'fileId and sharedWithId required' })

    // Ensure the requester is the owner (sharedById)
    const existingShares = await prisma.fileShare.findMany({ where: { fileId, sharedWithId } })
    if (!existingShares || existingShares.length === 0) return res.status(404).json({ error: 'Share not found' })

    // Ensure the requester is the owner for at least one of the shares
    const isOwner = existingShares.some((es) => es.sharedById === req.user?.userId)
    if (!req.user?.userId || !isOwner) return res.status(403).json({ error: 'Forbidden' })
    ...
    try {
      ...
      logger.debug('Socket emit failed for revoke', { err: (emitErr as Error).message })
    }
